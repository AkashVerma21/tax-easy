...
const ServicesCard = ({ services }) => {
  ...
  return (
    ...
    >
      ...
      {services?.price && (
        <p className={Classes.cardPrices}>
          {" "}
          @ INR{" "}
          <span style={{ textDecoration: "line-through" }}>
            {services?.price?.offerPrice}
          </span>{" "}
          <span>{services?.price?.listingPrice}</span> /-
        </p>
      )}
      {isMobile ? (
        list?.map((listItem, index) => (
          <ListContainer
            ...
            solution={listItem}
            className={{ servicesCardContainer: Classes.servicesCardContainer }}
          />
        ))
      ) : list?.length < 8 ? (
