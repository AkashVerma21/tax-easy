...
import {ITR_FAQ, itrCustomerInfo, itrFormsData, tableData, CitiesData, AssociatedWithData, itrStepList, itrDueDatesData, itrCustomerReview, itrFilingInCities,TeamPhotos, Home_Services} from '../../utils/constants/constantData'
  ...
  return (
    <div>
      ...
      <div className={Classes.itrFormsmMainContainer}>
            ...
            <p className={Classes.mainDescription}>We will guide you as which ITR Form is applicable to you as per your details</p>
      ...
      <div className={Classes.tableMainContainer}>
            ...
            <p className={Classes.mainDescription}>Old Regime or New Regime  (AY 2023-24)</p>
            <Table responsive bordered className={Classes.itrTable}>
                <thead>
                    <tr>
                        {tableData?.head?.map((heading, index)=><th key={`table-heading-${index}`}>{heading.split(' ').map((data,index)=>{ return (<><span key={`heading-${index}`}>{data}</span><br/></>)})}</th>)}
                    </tr>
                </thead>
                <tbody>
                      {tableData?.body?.map((desc,index)=>(
                        <tr key={`table-row-${index}`}>
                            ...
                            {Array(11).fill('*')?.map((info,idx)=><td className={index===idx? Classes.bgGreen : ''} key={`table-data-${idx}`}>{index===idx? <span>Same</span> : index<idx? <span className={Classes.blueColor}>Old</span> : <span className={Classes.greenColor}>New</span>}</td>)}
                        </tr>))}
                  </tbody>
              </Table>
      </div>
      <div className={Classes.calenderComponentContainer}>
            <h3 className={Classes.mainHeading}>ITR Filing Due Dates (AY 2023-24)</h3>
            <div className={Classes.cardMainContainer}> 
                <div>
                    <div className={classNames(Classes.dateContainer, Classes.dateContainerFirst)}>31st July 2023</div>
                    <DateCardContainer info={itrDueDatesData[0]} key={`itr-date-card-1`}/>
                </div>
                <div>
                    <div className={classNames(Classes.dateContainer, Classes.dateContainerSecond)}>31st October 2023</div>
                    <DateCardContainer info={itrDueDatesData[1]} key={`itr-date-card-2`}/>
                </div>
                <div>
                    <div className={classNames(Classes.dateContainer, Classes.dateContainerThird)}>30th November 2023</div>
                    <DateCardContainer info={itrDueDatesData[2]} key={`itr-date-card-3`}/>
                </div>
            </div>
      </div>
      ...
      <div className={Classes.betterTaxRegimeMainContainer}>
            ...
            <h3 className={Classes.heading}>Worry about which Tax Regime is Better for You ?</h3>
            <p className={Classes.secondDescription}>Need not to worry just, Just Call or Whatsapp and Talk to an Expert !</p>
            <a target='_blank' rel="noreferrer" href='tel:+91-9936818000' className={Classes.button}>CALL  NOW</a>
      </div>
      <FAQComponent heading={'Do you also have any of the following Scenario’s?'} description={'We have got you covered ! Call Now !'} QuestionsList={ITR_FAQ}/>
      <ITRFilingInCities/>
  ...
  return(
    <div className={Classes.itrFilingMainContainer} ref={vedioRef}>
            ...
            <div className={Classes.textContainer}>
                  ...
                  <input type="number" name="mobile" placeholder='Enter Mobile Number ' className={classNames(Classes.mobileInput, error? Classes.errorBorder : '')} value={mobileNo} onChange={(e)=>{setMobileNo(e.target.value); setError('')}}/>
                  {isMobile && <p className={Classes.errorMsg}>{error}</p>}
                  <button className={Classes.mobileInputButton} onClick={onSubmitClick}>GET INSTANT<br/>QUOTE!</button>
                  </div>
                  {!isMobile && <p className={Classes.errorMsg}>{error}</p>}
                  ...
                  <a target='_blank' rel="noreferrer" href='tel:+91-9936818000' className={Classes.callingNumber}>
    ...
    {formsData?.map((info, index)=>(
            <div className={Classes.formCard} key={`itr-forms-data-${index}`}>
              ...
              <div className={Classes.cardContainer}>
                  {info?.description1?.heading && <h6 className={Classes.cardHeading}>{info?.description1?.heading}</h6>}
                  <ul className={Classes.cardDescription}>
                    {info?.description1?.list?.map((list,idx)=><li key={`formsData-description1-${idx}`}>{list}</li>)}
                  </ul>
              </div>
              {info?.heading2 && <h5 className={Classes.heading}>{info?.heading2}</h5>}
              {info?.description2 && <div className={Classes.cardContainer}>
                  {info?.description2?.heading && <h6 className={Classes.cardHeading}>{info?.description2?.heading}</h6>}
                  <ul className={Classes.cardDescription}>
                    {info?.description2?.list?.map((list,idx)=><li key={`formsData-description2-${idx}`}>{list}</li>)}
                  </ul>
              </div>}
            </div>
          ))}
...
const DateCardContainer = ({info})=>{
  return (
    <div className={Classes.cardContainer}>
          ...
          <ul className={Classes.cardDescription}>
              {info?.list?.map((data, index)=><li key={`itr-filling-${index}`}>{data}</li>)}
          </ul>
      </div>
  )
}
      ...
      <Row className={Classes.descriptionContainer}>
        {itrFilingInCities?.map((dataList,index)=>(
          <Col key={`itrCitiesCol-${index}`} className={Classes.listContainer} xs={12} sm={6} md={6}>
            <ul>
              {dataList?.map((data,index)=><li key={`itrCitiesData-${index}`} onClick={()=>handleClick(data)}><Link to={`/${data.split(" ").join("-"," ")}`}>{data}</Link></li>)}
            </ul>
          </Col>
        ))}
      </Row>
